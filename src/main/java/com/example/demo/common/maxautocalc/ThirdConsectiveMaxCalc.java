...
public class ThirdConsectiveMaxCalc implements MaxCalcInterface {
	...
	public List<TIntervalValues> createTags(List<TIntervalValues> values, int consective) {
		List<TIntervalValues> tags = new ArrayList<TIntervalValues>();
	...
	public boolean compareTagValue(List<TIntervalValues> values, int idx, List<TIntervalValues> tags, int consective) {
		if(values.get(idx-1).getIntervalValue() >= tags.get(consective-3).getIntervalValue()
				&& values.get(idx-2).getIntervalValue() >= tags.get(consective-2).getIntervalValue()
				&& values.get(idx-3).getIntervalValue() >= tags.get(consective-1).getIntervalValue()) {
			return true;
		}
		return false;
	}
	...
	public List<TIntervalValues> createMaxValueList(List<TIntervalValues> values, List<TIntervalValues> tags, int consective) {
		...
		for(int idx=values.size() ;idx>=consective; idx--) {
			if(compareTagValue(values, idx, tags, consective)) {
	...
	public TIntervalValues createStepDownMaxValue(List<TIntervalValues> values, List<TIntervalValues> tags, int consective, int stepdown) {
		...
		while(jugdeTags(tags, consective)) {
			...
			//2連次の基準値tag1とtag2とtag3を下げる
			tags.get(consective-3).setIntervalValue(
					subTag(tags.get(consective-3).getIntervalValue()));
			tags.get(consective-2).setIntervalValue(
					subTag(tags.get(consective-2).getIntervalValue()));
	...
	public boolean jugdeTags(List<TIntervalValues> tags, int consective) {
		if(tags.get(consective-2).getIntervalValue() >= 0
				&& tags.get(consective-1).getIntervalValue() >= 0) {
			return true;
		}
		return false;
	}
