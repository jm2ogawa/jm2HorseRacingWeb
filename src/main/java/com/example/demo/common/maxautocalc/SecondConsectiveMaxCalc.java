 ...
 *
 */
public class SecondConsectiveMaxCalc implements MaxCalcInterface {
	...
	public boolean compareTagValue(List<TIntervalValues> values, int idx, List<TIntervalValues> tags, int consective) {
		if(values.get(idx-1).getIntervalValue() >= tags.get(consective-2).getIntervalValue()
				&& values.get(idx-2).getIntervalValue() >= tags.get(consective-1).getIntervalValue()) {
			return true;
		}
		return false;
	}
